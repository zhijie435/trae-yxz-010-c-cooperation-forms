...
import { ref } from 'vue'
import IncubationApplyForm from '@/components/IncubationApplyForm.vue'
  ...
  <div class="grain relative min-h-screen w-full overflow-x-hidden">
    ...
    >
      <header class="flex items-center justify-between animate-rise">
        <div class="flex items-center gap-3">
          ...
          <div class="leading-tight">
            <p class="font-serif text-[15px] font-semibold text-ink">产业孵化申请</p>
            <p class="text-[10px] uppercase tracking-[0.22em] text-muted">Industry Incubation</p>
          </div>
        </div>
        <span
      ...
      <section class="mt-12 animate-rise" style="animation-delay: 80ms">
        <p class="mb-3 text-[11px] uppercase tracking-[0.3em] text-gold">Incubation Application</p>
        ...
        <p class="mt-4 max-w-xl text-sm leading-relaxed text-ink-soft">
          填写您的项目介绍与孵化需求，我们将为您对接专业的产业孵化资源，助力项目快速成长。提交后将生成专属回执编号，便于后续跟进。
        </p>
      ...
      <main class="mt-8 flex-1">
        ...
        <IncubationApplyForm v-else @success="onSuccess" />
      </main>
      ...
      >
        <span>© 2026 产业孵化计划</span>
        <span>提交即代表同意信息用于孵化审核</span>
      </footer>
    </div>
  </div>
</template>
